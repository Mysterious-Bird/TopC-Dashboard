...
import { contestStatus, type Contest } from '../data/mock'
import { addDaysIso, chinaTodayIso, formatChinaDateTime } from '../data/date'
...
interface ScheduleItem {
  ...
  label: string // 如「赛前 7 天」「报名催办」
  recipientsDesc: string
  sentLog?: ReminderLogItem
}

function recipientsDescOf(c: Contest) {
  if (c.remindRecipientIds.length > 0) return `指定 ${c.remindRecipientIds.length} 人`
  return `参赛者 ${c.participantIds.length} 人 + 社长`
}
...
export default function Reminders() {
  ...
  // 排期预览：赛前节点（发送日 = 开赛日 - N）+ 报名催办（发送日 = 截止日 - 3）+ 固定事项
  const schedule = useMemo<ScheduleItem[]>(() => {
    ...
    for (const c of contests) {
      ...
      if (st === '已结束') continue
      const raceDesc = recipientsDescOf(c)
      if (c.remindEnabled) {
        for (const n of [...c.reminderDays].sort((a, b) => b - a)) {
          const date = addDaysIso(c.start, -n)
      ...
      if (c.registerBy) {
        const date = addDaysIso(c.registerBy, -3)
  ...
  const todayIso = chinaTodayIso()

  return (
    <div className="mx-auto max-w-[1200px] px-4 py-4 sm:px-6 sm:py-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <PageTitle title="提醒中心" sub="赛前节点自动邮件 · 报名催办 · 固定日期事项提醒" />
        {authed && (
          <button
            onClick={scan}
            disabled={scanning}
            ...
            {logs.map((l) => (
              <div key={l.id} className="border-b border-edge/50 px-5 py-3 last:border-0">
                ...
                <div className="mt-1 flex items-center justify-between text-[12px] text-ink-3">
                  <span className="font-mono">{formatChinaDateTime(l.sent_at)}</span>
                  <span>
